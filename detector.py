...
class Detector(torch.nn.Module):
    ...
    def __init__(self):
        ...
        if os.path.exists("model/darknet0916.pth"):
            self.net.load_state_dict(torch.load("model/darknet0916.pth"))
...
def make_squre(im, max_size=300):
    im = Image.open(im)
    x, y = im.size
    scale = max(x, y) / 300
...
if __name__ == '__main__':
    ...
    img, new_img, scale, dx, dy = make_squre("data04/JPEGImages/0040.jpg")
    ...
    if y.shape[0] != 0:
        ...
        labels = {0: "car", 1: "bus", 2: "motorcycle"}
